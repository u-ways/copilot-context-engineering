...
1. List the repository's Markdown files with `git ls-files '*.md'`, excluding anything under `.github/` and `.claude/` and any `CLAUDE.md` or `AGENTS.md` at the repository root: those are assistant configuration, not framework pages. That listing is the only shell command you run before the audit.
...
5. Reply with only two lines: the number of files without such a section as a bare integer, then one sentence stating the scope (how many framework pages were examined, what was excluded, and the fence rule).

...
- `cce doctor [--offline] [--json] [--verbose]` reports `ok|warn|fail  name  detail` rows for: Python version, git presence and version, `copilot` presence and version (1.0.83 or newer recommended), `uv`, global Copilot customisation that can skew scenarios (`copilot-instructions.md`, `skills`, `agents` and `hooks` under `$COPILOT_HOME`, else `~/.copilot`), whether every overlay renders against placeholder upstream text, and upstream reachability (skipped with `--offline`). `--verbose` prints every file behind a warn row beneath it; `--json` carries them as `details`. Nothing about Claude Code or herdr is reported. An absent workspace is never a failure.

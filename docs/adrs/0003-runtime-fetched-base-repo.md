...
The scenarios are built on a real repository so that what a reader sees in `/context`, in token counts and in diffs comes from real pages rather than invented filler. The base content is `NHSDigital/software-engineering-quality-framework`. That repository carries no licence, which means no right to copy, modify or redistribute any part of it: no file, snapshot or excerpt from it may be committed here, shipped in the wheel, or uploaded from CI.
...
- If upstream deletes or rewrites history at the pin, setup on a fresh machine breaks. There is no mirror, by design, because a mirror would be redistribution.
...
- Flag any file under `tests/` outside `tests/e2e/` and `tests/llm/` that contains `NHSDigital/` or clones from a URL that does not start with `file://`.

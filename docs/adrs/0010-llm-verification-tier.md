...
This ADR is Proposed rather than Accepted. The LLM tier itself (`tests/llm/`, `just llm`, `llm-tests.yml`) lands in a later pull request; today only the `llm` pytest marker and the dialect translator exist. The pull request that delivers the tier flips this ADR to Accepted.
...
- Runs are isolated from personal configuration: `COPILOT_HOME=.cce-artifacts/home-copilot` and `CLAUDE_CONFIG_DIR=.cce-artifacts/home-claude` when the corresponding token is set. `.cce-artifacts/` is gitignored.
...
- The token metric is the main thread's final call context, which is what the context-cost scenarios (S02, S06) claim to change; subagent usage is deliberately excluded.
...
- Until the tier lands this ADR records intent only; the `llm` marker and the dialect translator are the parts that exist today.

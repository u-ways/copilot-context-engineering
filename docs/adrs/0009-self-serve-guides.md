...
- Status: Accepted
- Date: 2026-09-13
- Revision 2026-09-13: the presenter guide lives at `docs/slides/presenting.md`, next to the deck and its theme; the force-include maps it from there and the `docs/` fallback reads it from there.
- Revision 2026-09-13: the comparison table with the TL;DR line lives in `docs/SUMMARY.md`, where every row links the scenarios that demonstrate it; `docs/scenarios/README.md` carries the scenario list with a link to every guide. `README.md` is the front page and carries no content required by this ADR.
...
- One guide per scenario at `docs/scenarios/<slug>.md`, where `<slug>` is the id-prefixed manifest slug (`03-skills-on-demand.md`), plus `docs/scenarios/README.md`, the walkthrough index that links every guide, and `docs/slides/presenting.md` for live talks. No other files live under `docs/scenarios/`.
...
- The guides ship inside the wheel: `[tool.hatch.build.targets.wheel.force-include]` maps `docs/scenarios` to `cce/guides` and `docs/slides/presenting.md` to `cce/guides/presenting.md`. `cce guide ID|presenting|scenarios` prints a guide, the presenter guide or the walkthrough index to stdout; the loader uses the packaged directory when it exists and falls back to `docs/` in an editable install, where hatchling's force-include is not visible.
- `docs/SUMMARY.md` carries the "Which should I use?" comparison table with the TL;DR line, and every row links the scenario guides that demonstrate it. `docs/scenarios/README.md` lists scenarios 01–06 with links to the guides.
...
- Require `docs/SUMMARY.md` to contain the rows `| Always loaded? |`, `| Adaptive? |` and `| Size concern? |`, each linking at least one `scenarios/<slug>.md` guide, and the line starting `TL;DR: Instructions = "Always follow these rules."`.
...
- Require `[tool.hatch.build.targets.wheel.force-include]` in `pyproject.toml` to map `docs/scenarios` to `cce/guides` and `docs/slides/presenting.md` to `cce/guides/presenting.md`.
- Require `src/cce/manifest.py` to fall back to `docs/` when the packaged `guides` directory is absent.
- Flag any `docs/scenarios/*.md` or `docs/slides/presenting.md` line that quotes upstream prose (the e2e licence guard is the mechanical check).

...
- One guide per scenario at `docs/scenarios/<slug>.md`, where `<slug>` is the id-prefixed manifest slug (`03-skills-on-demand.md`), plus `docs/presenting.md` for live talks. No other files live under `docs/scenarios/`.
...
- The guides ship inside the wheel: `[tool.hatch.build.targets.wheel.force-include]` maps `docs/scenarios` to `cce/guides` and `docs/presenting.md` to `cce/guides/presenting.md`. `cce guide ID|presenting` prints a guide to stdout; the loader uses the packaged directory when it exists and falls back to `docs/` in an editable install, where hatchling's force-include is not visible.
...
- Require `docs/scenarios/<slug>.md` for every `[[scenario]]` slug in `src/cce/overlays/scenarios.toml`, and no other `.md` files under `docs/scenarios/`.
- Require every `docs/scenarios/*.md` file to contain exactly the level-2 headings `## What it shows`, `## Run it`, `## What to notice` and `## Reset`, in that order.
